...
trash ~/Documents/personal/default/dotfiles/book* &>/dev/null
trash "$HOME/Downloads/user.js" &>/dev/null
trash "$HOME/Downloads/userMY.js" &>/dev/null

latest=$(find ~/.mozilla/firefox/*.default/bookmarkbackups -type f -name '*.jsonlz4' -printf '%T@ %p\n' |
  sort -nr |
  awk 'NR==1 {print substr($0, index($0,$2))}')
cp -f "$latest" ~/Documents/personal/default/dotfiles/
...
' "$HOME/Documents/personal/default/dotfiles/user.js" >"$HOME/Downloads/userMY.js"
