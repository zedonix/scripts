...
if [ -z "$cliphist_list" ]; then
    fuzzel -d --width=100 --placeholder "cliphist: please store something first" --lines 0
    rm -rf "$thumbnail_dir"
    exit
fi
...
item=$(echo "$cliphist_list" | gawk "$thumbnail" | fuzzel -d --width=100 --placeholder "Search clipboard..." --counter --no-sort --with-nth 2)
exit_code=$?

# ALT+0 to clear history
if [ "$exit_code" -eq 19 ]; then
    confirmation=$(echo -e "No\nYes" | fuzzel -d --width=100 --placeholder "Delete history?" --lines 2)
